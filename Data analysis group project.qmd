...
bmi.model1 <- lm_spec |> fit(BMI ~ Age + Sex + Education + Veg + Fruit +Year, data = bmi.data)
bmi.model1 <- bmi.model1 |> extract_fit_engine()
get_regression_table(bmi.model1)

lm_spec <- linear_reg() 
bmi.model1.int <- lm_spec |> fit(BMI ~ Age * Sex * Education * Veg * Fruit * Year, data = bmi.data)
...
bmi_full_model <- lm_spec %>% 
  fit(BMI ~ Age + Sex + Education + Veg + Fruit + Year, data = bmi.data)
