...
```{r}
ggplot(bmi.data, aes(sample = BMI)) +
  geom_qq() +
  geom_qq_line() +
...
From the regression table @tbl-full-model, we can see that the significance of the variables "Sex: Male", "Veg: Yes" and "Fruit: Yes" all include 0 in their confidence intervals and have a p-value that exceeds 0.05. This causes us to question whether these variables are significant in telling us anything about the BMI distribution. We will remove them from the model:
...
From Table1, we can see that all 3 models have low Adjusted R squared (\< 0.1) which would indicate that that the models do not describe the variation in the data very well. The model that maximises adjusted R squared and minimizes the AIC and BIC is the interaction model:
